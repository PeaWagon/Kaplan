...
\wss{Only include the units that your SRS actually uses} \wss{You can remove
  these comments from the blank template file.}
...
$c_i$ & basis set constant \\
$c_N$ & normalization/principle quantum number constant \\
$\zeta$ & effective nuclear charge constant \\
\ce{e-} & N/A & electron \\
x & x-component of Cartesian coordinates \\
y & y-component of Cartesian coordinates \\
z & z-component of Cartesian coordinates \\
$Z_\alpha$ & atomic number \\
\end{longtable*}
...
\wss{Add any other abbreviations or acronyms that you add} \wss{Why is the
  comment ``Kaplan'' above?  You should say that Kaplan is your program's name,
  not just repeat the term.  Also, why is it a comment from me?} \wss{I like how
  you split the list of abbreviations into domain areas.}
...
find the local minima of this molecule. \wss{I am confused by the difference
  between the global minimum and the local minima.  Are you saying that there
  are times when the local minima is actually a better solution than the global
  one?  If that is what you are saying, then I guess I'm not confused, but it
  goes against the usual rules of optimization.   You are usually looking for
  the global minimum and trying to avoid getting stuck in a local minimum.} Knowing the 
...
\wss{The above comments from the original template are distracting.  You should
  delete them or at least comment them out.  However, they do remind me of one
  thing, in your introduction you should cite the source of the template that
  you are using.  I just saw that the template is cited in the ``Organization of
the Document'' section; you can ignore my previous statement.}
...
\meow{Does the wording here imply that none of these definitions can be used in
  previous sections? Should the early sections of this document be so abstract
  as to not use any terminology?} \wss{You might want to change the colour of
  your comments.  They are hard to read on the pdf.} \wss{No, although I can see
  why you would ask.  You can use these terms previously, but you wouldn't rely
  on the exact definitions.  You would use the terminology in a less careful way
...
covalent bond here?} \wss{Great question.  If the intended reader should know
the terminology, then you don't have to define it.  However, defining something
they likely already know, is still okay.  Just because you can leave something
out, doesn't mean that you should.  You will also have readers, like me, that
aren't familiar with all of the terminology.  If it isn't too much work,
including terminology is a good idea.  Also, some terminology is used
ambiguously, even be experts, so picking the definition you are using and making
it unambiguous is worthwhile.}
...
\wss{I am not in a position to judge the above text, but it sounds great!}
\wss{I didn't check all of them, but it is great to see that the acronyms I
  checked are in your table of acronyms.}
