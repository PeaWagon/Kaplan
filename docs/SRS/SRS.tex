...
May 21, 2019 (Tuesday) & 1.5 & update to the current state of the code; remove references to z-matrices \\
\bottomrule
\end{tabularx}
...
The program will look for a set of conformers that are geometrically distinct
and stable (meaning they have a relatively large, negative energy). To generate
a conformer, the program will start with a full geometry specification. Then,
the dihedral angles that are most integral to the molecule's backbone will
be selected for optimisation. These dihedral angles will be manipulated as to
explore the energetic landscape. 
...
are from one another. The RMSD measure is necessary, otherwise the user could get 3 
...
\begin{itemize}
\item \textbf{covalent bond:} a connection that involves sharing electronic
density between two atoms.
\item \textbf{ionic bond:} a connection that is the result of electrostatic
attraction that occurs between a cation (positively charged ion) and an anion
...
\item \textbf{internal coordinates:} a way to specify how atoms are connected (relative
to one another), such through bond lengths, bond angles, dihedral angles.
\item \textbf{cartesian coordinates:} a mapping of atoms in space using 3-dimensions,
centred about the origin (0,0,0).
...
\item[PS1:] A molecular identifier. Describes the molecule in a unique way. Examples
include: name, InChiKey, and SMILES string. 
\item[PS2:] A molecular geometry. Describes how the atoms in the molecule are
explicitly connected/related in 3-dimensional space. Includes cartesian and internal
coordinates.
\item[PS3:] A set of dihedral angles. Can be extracted from a geometry. Some dihedral
angles are more important than others (i.e. make a larger change in the energy when
manipulated).
...
\begin{table}[H]
	\begin{tabular}{lll}
		\toprule
		\textbf{XYZ (Cartesian)} & 
		\textbf{SMILES} & InChiKey \\
	  \textbf{Coordinates} & \textbf{String} & \\
		\hline
	14		&  CCCC & IJDNQMDRQITEOD-UHFFFAOYSA-N\\
	butane  		&	& \\
  C -0.5630  0.5160  0.0071 &	& \\
  C  0.5630 -0.5159  0.0071	&	& \\
	C -1.9293 -0.1506 -0.0071	&	& \\
	C  1.9294  0.1505 -0.0071	&	& \\
	H -0.4724  1.1666 -0.8706	&	& \\
	H -0.4825  1.1551  0.8940	&	& \\
	H  0.4825 -1.1551  0.8940	&	& \\
	H  0.4723 -1.1665 -0.8706	&	& \\
	H -2.0542 -0.7710 -0.9003	&	& \\
	H -2.0651 -0.7856  0.8742	&	& \\
	H -2.7203  0.6060 -0.0058	&	& \\
...
molecule. Dihedral angles can be extracted from the cartesian coordinates. During optimization, it is 
important that the 
...
During the optimisation, the potential solutions are represented by sets of dihedral 
angles. Each solution instance is therefore a set of geometries. The size of 
the set of geometries is specified at runtime by the user. For example, the 
user requests 3 conformers and a population size of 100, then there will be 
300 total sets of dihedral angles for the input molecule. It may be that some 
of the geometry specifications do not allow the energy to be calculated (for 
example, a dihedral angle causes two atoms to overlap). In these cases, the fitness 
will be very small for the solution instance, as one or more of the geometries 
is not viable.
...
conformer space is independent of the ordering of the input atoms in a cartesian 
coordinate system. This may 
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
	Input&$n_G$ sets of dihedral angles, each with an associated energy, 
	$C_E$, $C_\text{RMSD}$, $S_E$, and $S_\text{RMSD}$ \\
